...
### 2. 高精度 GPS 位置模拟与轨迹回放
- **即时获取手机真实定位**：自动申请定位权限，启动即自动搜星定位，右上角一键居中到当前位置，地图常驻“我的位置”光晕蓝点；
- **户外真实轨迹录制（新特性）**：支持带着手机实际走动/骑行/驾车，一键开启「录制轨迹」，内置智能防抖滤波采样（自动剔除飘移噪点与静止漂移），实时统计航点、里程与速度，录制完成后可一键「应用为模拟轨迹」或「导出为 GPX 文件」；
...
4. **启动位置模拟**：
   ...
   - 自动获取真实定位后，点击右上角定位图标即可随时定位回当前位置；
   - **手动绘制**：开启「标记」，在地图上点击添加轨迹点，或点击「导入 GPX」；
   - **现场录制**：点击「录制轨迹」按钮，手持设备外出走动即可实时采样真实航迹，完成后可一键「应用为模拟轨迹」或「导出 GPX」；
